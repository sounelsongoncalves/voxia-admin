...
export const DriverDetail: React.FC = () => {
  ...
  const handleStatusChange = async (newStatus: Status) => {
    ...
    try {
      ...
      showToast('Erro ao atualizar status do motorista.', 'error');
    }
  };

  const handleSuspend = () => {
    if (window.confirm('Tem certeza que deseja suspender este motorista?')) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-96">
        <p className="text-txt-tertiary">Carregando detalhes do motorista...</p>
      </div>
    );
  }
  ...
  if (!driver) {
    return (
      <div className="flex flex-col items-center justify-center h-96 gap-4">
        <p className="text-txt-tertiary">Motorista não encontrado</p>
        <button
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2 text-sm text-txt-tertiary mb-2">
        ...
        <span className="cursor-pointer hover:text-txt-primary" onClick={() => navigate('/drivers')}>Motoristas</span>
        <span className="material-symbols-outlined text-xs">chevron_right</span>
      ...
      <div className="bg-surface-1 border border-surface-border rounded-xl p-6">
        <div className="flex flex-col md:flex-row gap-6 items-start">
          ...
          <div className="bg-surface-2 p-4 rounded-xl border border-surface-border min-w-[200px]">
            <p className="text-xs text-txt-tertiary uppercase font-bold mb-2">Score de Segurança</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-surface-1 border border-surface-border rounded-xl p-6">
          <h3 className="text-lg font-bold text-txt-primary mb-4">Histórico de Viagens Recentes</h3>
          <div className="space-y-4">
            {trips.length === 0 ? (
              <p className="text-sm text-txt-tertiary text-center py-4">Nenhuma viagem recente</p>
            ) : (
              trips.map(trip => (
        ...
        <div className="bg-surface-1 border border-surface-border rounded-xl p-6">
          <h3 className="text-lg font-bold text-txt-primary mb-4">Documentação</h3>
          <div className="space-y-3">
            <div className="flex justify-between items-center p-3 rounded-lg bg-surface-2 border border-surface-border">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-bold text-txt-primary">Carta de Condução ({driver.license_category || 'E'})</p>
                  <p className="text-xs text-txt-tertiary">Vence em {driver.license_expiry || '12/2025'}</p>
                </div>
              </div>
              <span className="text-semantic-success text-xs font-bold">Válida</span>
            </div>
            <div className="flex justify-between items-center p-3 rounded-lg bg-surface-2 border border-surface-border">
              <div className="flex items-center gap-3">
                <span className="material-symbols-outlined text-brand-primary">verified_user</span>
                <div>
                  <p className="text-sm font-bold text-txt-primary">Exame Toxicológico</p>
                  <p className="text-xs text-txt-tertiary">Realizado em 01/2024</p>
                </div>
              </div>
              <span className="text-semantic-success text-xs font-bold">OK</span>
            </div>
          </div>
        </div>
      </div>
